...
> **ステータス: ベータ（Windows専用）。** 検出は完全ではありません。出力の公開可否は必ず人間が最終確認してください。監査のPassは「設定された検査を通過した」ことを意味し、絶対的な安全を保証するものではありません。保証範囲と責任分担の詳細は [品質保証と責任範囲](QUALITY.jp.md)、プライバシーは [プライバシーポリシー](PRIVACY.jp.md)（何も収集しません）を参照してください。

![レビュー画面: 架空のテスト文書上の検出候補](images/review.png)

![適用後: 表記揺れが一貫した仮名にその場で置換される](images/applied.png)
...
- **日英二言語UI** — 既定ではOSの言語に従い（日本語環境は日本語、それ以外は英語）、設定ダイアログで明示的に切り替えられます。検出そのものは日本語文書向けです。
...
# 以降はプロジェクトルートから起動できます
.\scripts\dev.ps1
```

`scripts/dev.ps1` はプロジェクトルートから開発サーバーを起動し、終了時に自動でルートへ戻ります。開発サーバーは、ターミナルで Ctrl+C せず**アプリのウィンドウを閉じて終了**してください（Windows では Ctrl+C だと cmd のバッチ中断となります）。このスクリプトは起動・終了の両方で Vite の port 1420 を解放するので、開発サーバーが残って次回の起動を妨げることはありません。
